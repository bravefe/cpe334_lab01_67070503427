...
describe("Lab 3 user management", () => {
  ...
  it("UI-18: shows duplicate email error below Email Address", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Create User" }));
    await user.type(screen.getByLabelText("Full Name"), "New Person"); await user.type(screen.getByLabelText("Email Address"), "elrond@example.com"); await user.type(screen.getByLabelText("Initial Password"), "Password123!"); await user.click(screen.getByRole("button", { name: "Save User" }));
  ...
  it("UI-20: prefills the edit panel", async () => {
    ...
    expect(screen.getByDisplayValue("Arwen Staff")).toBeInTheDocument(); expect(screen.getByDisplayValue("arwen@example.com")).toBeInTheDocument(); expect(screen.getByDisplayValue("IT Staff")).toBeInTheDocument();
  });
  it("UI-29: keeps last-admin conflict in the open panel", async () => {
    ...
    await user.selectOptions(screen.getAllByRole("combobox")[1], "REQUESTER"); await user.click(screen.getByRole("button", { name: "Save User" }));
