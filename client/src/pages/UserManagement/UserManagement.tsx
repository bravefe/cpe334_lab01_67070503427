    ...
    <>
      ...
      <main className="page users-page">
        <header className="page-header">
          <div>
            <h1>Users</h1>
            <p className="muted">Create and manage user accounts.</p>
          </div>
          <button
            className="primary"
            type="button"
        ...
        {status === "loading" && (
          <section className="ticket-panel users-loading">
            Loading users...
          </section>
        )}
        {status === "error" && (
          <section className="empty">
            <h2 className="error-message">Unable to load users.</h2>
        ...
        {status === "ready" && users.length > 0 && (
          <section className="ticket-panel users-table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Name</th>
              ...
              <tbody>
                {users.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span
                        className={`user-role role-${item.role.toLowerCase()}`}
                      >
                        {roleName(item.role)}
                      </span>
                    </td>
                    <td>
                      <span
                        className={`user-status ${item.isActive ? "active" : "inactive"}`}
                      >
      ...
      {editing && (
        <UserPanel
          user={editing === "create" ? null : editing}
...
function UserPanel({
  ...
  const create = !user;
  const [form, setForm] = useState<UserInput & { initialPassword: string }>(
    user ? { ...user, initialPassword: "" } : emptyForm(),
  );
  ...
  const validate = (password = create) => {
    ...
    if (password && form.initialPassword.length < 8)
      next.initialPassword = "Password must be at least 8 characters.";
    setErrors(next);
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    try {
      if (create) await createUser(form);
      else await updateUser(user.id, form);
      onSaved(create ? "User created." : "User updated.");
    } catch (error) {
      const api = error as ApiError;
      if (api.code === "DUPLICATE_EMAIL") setErrors({ email: api.message });
      else setConflict(api.message);
    } finally {
      setSaving(false);
    }
  };
  const reset = async () => {
    ...
    setResetting(true);
    try {
      await resetUserPassword(user!.id, form.initialPassword);
      onSaved("Password reset — user must change it at next login.");
    } catch (error) {
      setErrors({
        initialPassword:
  ...
  return (
    <div className="user-overlay" role="presentation">
      <aside
        className="user-panel"
        aria-label={create ? "Create New User" : "Edit User"}
      >
        <header>
          <h2>{create ? "Create New User" : "Edit User"}</h2>
        ...
        <form autoComplete="new-password" onSubmit={submit} noValidate>
          ...
          {conflict && (
            <p className="panel-conflict" role="alert">
              {conflict}
            </p>
          )}
