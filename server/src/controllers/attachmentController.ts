...
export async function removeAttachment(req: Request, res: Response) {
  const id = req.user!.id; const reason = typeof req.body?.reason === "string" ? req.body.reason.trim() : "";
  if (!reason) return error(res, 400, "VALIDATION_ERROR", "Removal reason is required.");
